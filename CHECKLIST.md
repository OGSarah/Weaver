...
- [ ] Install Go (`brew install go`) and confirm with `go version`.
...
- [ ] Create the repo: `go mod init github.com/<you>/weaver`.
...
- [ ] Define Go structs for a workflow: a set of tasks and the dependency edges between them.
...
- [ ] Choose a migration tool (golang-migrate is a common choice) and wire it in.
